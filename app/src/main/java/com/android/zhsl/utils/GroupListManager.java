...
import com.android.zhsl.bean.DeviceInfo;
import com.android.zhsl.bean.TreeNode;
import com.android.zhsl.exception.IDpsdkCoreException;
...
//        }
     ...
     * @return
     */
    public synchronized TreeNode getGroupList(byte[] coding, TreeNode curNode)  {
        if (curNode == null)
            return curNode;
      	Boolean hasLogicorg = IDpsdkCore.DPSDK_HasLogicOrg(getnPDLLHandle());
      	if (hasLogicorg) {
            ...
            for (int i = 0; i < depNum.nReturnValue; i++ ) {
                ...
                //递归查找组织
                getGroupList(logicDepNode.getValue().getBytes(), logicDepNode);
                if(logicDepNode.getChildren().size() == 0){
                	continue;
                }
            ...
            Log.i(TAG, "cNum" + cNum);
            TreeNode devNodes = addChannelInfoNode(ecInfo, curNode);
            throwException("DPSDK_GetLogicDepNodeNum_EXCEPTION");
...
//                curNode.add(devNode);
                ...
                TreeNode devNodes = addChannelInfoNode(ecInfo, devNode);   //通道添加过程中，过滤到无权限通道
...
//        }
    ...
    private TreeNode addChannelInfoNode(Enc_Channel_Info_Ex_t[] ecInfo, TreeNode devNode) {
...
//              }
              ...
              IDpsdkCore.DPSDK_GetDeviceTypeByDevId(getnPDLLHandle(), szDevId, return_Value_Info_t);
              if (return_Value_Info_t.nReturnValue == dpsdk_dev_type_e.DEV_TYPE_NVR) {
            	  IDpsdkCore.DPSDK_QueryNVRChnlStatus(getnPDLLHandle(), szDevId, DPSDK_CORE_DEFAULT_TIMEOUT);
              } else {
              ...
              try {
                  ...
                  channelInfo.setLat(Latitude);
                  channelInfo.setLng(Longitude);
                  ...
                  L.d(TAG,e.toString());
                  channelInfo.setLat("0");
                  channelInfo.setLng("0");
              }
