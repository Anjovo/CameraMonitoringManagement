...
public class ActivityMap1 extends ActivityTitleBase{
...
//构建MarkerOption，用于在地图上添加Marker
        ...
        for (int i=0;i<showList.size();i++){
            if(ll_right.latitude<lat&&lat<ll_left.latitude&&ll_left.longitude<lng&&lng<ll_right.longitude){
                mBaiduMap.addOverlay(showList.get(i));
                MarkerOptions mOverlayOptions = (MarkerOptions) showList.get(i);
                Bundle bun = mOverlayOptions.getExtraInfo();
                VideoDataBean.child buns = (VideoDataBean.child) bun.getSerializable("bun");
                infoWindow(Double.parseDouble(buns.getLat()),Double.parseDouble(buns.getLng()),bun);
            }
        }
...
//                .getSystemService(Context.WINDOW_SERVICE);
        ...
        queren.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(nowPosition == -1){
                    T.showShort(ActivityMap1.this,"你还没有选择任何视屏通道");
                    return;
                }
                if("1".equals(views.getGrandson().get(nowPosition).getIsSelect())){
                    ...
                    StartToUrlUtils.getStartToUrlUtils().startToActivity(ActivityMap1.this, ActivityVideoPay.class,bun);
                }else{
                    T.showShort(ActivityMap1.this,"你还没有选择任何视屏通道");
                }
            }
